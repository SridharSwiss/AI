...
export const caseStudiesData1: CaseStudyData[] = [
  {
    ...
    outcome:
      "COIN now reviews documents in seconds that previously took lawyers hours. An estimated $150M+ in annual savings. Error rate reduced significantly, and lawyers can focus on complex judgment calls.",
    metrics: [
      "360,000 lawyer hours saved annually",
      "Estimated $150M+ in annual savings",
  ...
  },
  {
    ...
    company: "Mayo Clinic",
    industry: "Healthcare",
    title: "AI-Enabled ECG: Detecting Hidden Heart Disease from a Standard EKG",
    problem:
      "Asymptomatic left-ventricular dysfunction (a weak heart pump, or low ejection fraction) is common and treatable, but usually silent until it progresses to heart failure. Confirming it requires an echocardiogram, which is costly and not used for routine screening — so many cases go undetected.",
    solution:
      "Mayo Clinic researchers trained a deep convolutional neural network to read a standard, inexpensive 12-lead ECG and flag patients likely to have low ejection fraction (≤35%). A companion model detects atrial fibrillation from an ECG recorded during normal sinus rhythm. The approach was validated in a pragmatic randomized trial (EAGLE) and deployed into routine Mayo practice; it has been commercialized through Anumana (an nference venture).",
    outcome:
      "In the pivotal study (Attia et al., Nature Medicine, 2019), the AI-ECG identified low ejection fraction with an AUC of about 0.93. The EAGLE randomized trial (2021) showed the tool increased the rate of newly diagnosed low-EF patients in everyday clinical practice. The atrial-fibrillation-from-sinus-rhythm model reached an AUC of roughly 0.87 (Lancet, 2019).",
    metrics: [
      "~0.93 AUC detecting low ejection fraction from a 12-lead ECG",
      "EAGLE randomized trial: higher rate of newly diagnosed low-EF patients",
      "AFib detection from a normal-rhythm ECG at ~0.87 AUC",
      "Validated on millions of Mayo ECGs; commercialized via Anumana",
    ],
    tags: ["Healthcare", "Deep Learning", "Cardiology", "Diagnostics", "Medical AI"],
    featured: true,
    businessContext:
      "Mayo Clinic holds one of the world's largest digital ECG archives — tens of millions of ECGs paired with decades of linked clinical outcomes. This unique dataset let Mayo turn a ubiquitous, low-cost test into an AI screening tool, extending diagnosis of serious heart conditions far beyond the reach of expensive imaging and specialist review.",
    strategicDrivers: [
      "Low ejection fraction is common, treatable, and often silent — a major undetected-disease gap",
      "ECGs are cheap and ubiquitous; echocardiograms are not, limiting routine screening",
      "Mayo's decades-long ECG + outcomes archive is a rare, defensible training asset",
      "Opportunity to extend specialist-level cardiac screening to primary and rural care",
      "Commercialization path via Anumana/nference to scale beyond Mayo",
    ],
    techStack: [
      "Python / PyTorch",
    ...
    architecture:
      "Raw 12-lead ECG voltage-time waveforms (10 seconds sampled at up to 500 Hz) are fed into a convolutional neural network that outputs a probability of low ejection fraction. The model learns subtle waveform patterns imperceptible to clinicians. Outputs are surfaced to physicians alongside the standard ECG report; abnormal results prompt a confirmatory echocardiogram. The atrial-fibrillation model runs on ECGs recorded during normal sinus rhythm to flag patients with paroxysmal AFib.",
    dataRequirements:
      "Millions of retrospective 12-lead ECGs paired with echocardiogram-derived ejection-fraction labels drawn from Mayo's multi-decade archive. De-identified before model training; validated on held-out cohorts and in a prospective randomized trial (EAGLE) across multiple Mayo sites.",
    investmentEstimate: "Multi-year academic and platform investment (research, validation trials, and commercialization via Anumana/nference)",
    annualReturn: "Value is primarily clinical (earlier detection of treatable disease) rather than a single reported dollar figure",
    paybackPeriod: "Not publicly disclosed",
    roiMultiple: "Not publicly disclosed",
    roiBreakdown: [
      {
        category: "Earlier detection of treatable heart disease",
        value: "Clinical",
        note: "Identifying asymptomatic low ejection fraction earlier enables treatment before progression to heart failure",
      },
      {
        category: "Lower-cost screening at scale",
        value: "Clinical",
        note: "Turns a cheap, ubiquitous ECG into a screening test, avoiding routine reliance on costly echocardiograms",
      },
      {
        category: "Commercialization",
        value: "Platform",
        note: "Licensed and scaled beyond Mayo through Anumana (an nference venture)",
      },
    ],
    implementationTimeline: "Multi-year research-to-deployment: pivotal study 2019, EAGLE randomized trial 2021, commercialization thereafter",
    implementationPhases: [
      {
        phase: "Dataset Assembly & Labeling",
        duration: "—",
        description:
          "Assembled millions of historical 12-lead ECGs paired with echocardiogram-derived ejection-fraction labels from Mayo's multi-decade archive; de-identified for research use.",
        keyOutputs: ["Paired ECG + echo-label dataset", "De-identified research corpus", "Held-out validation cohorts"],
      },
      {
        phase: "Model Development & Publication",
        duration: "—",
        description:
          "Trained the convolutional network on ECG waveforms and validated it, reaching ~0.93 AUC for low ejection fraction. Published in Nature Medicine (2019); AFib-from-sinus-rhythm model published in the Lancet (2019).",
        keyOutputs: ["~0.93 AUC low-EF model", "Nature Medicine 2019 publication", "Lancet 2019 AFib model"],
      },
      {
        phase: "Prospective Trial & Deployment",
        duration: "—",
        description:
          "Ran the pragmatic EAGLE randomized trial across Mayo primary-care sites, showing increased diagnosis of low EF in routine practice, then deployed into the clinical workflow.",
        keyOutputs: ["EAGLE randomized trial (2021)", "Clinical workflow integration", "Commercialization via Anumana"],
      },
      {
  ...
  },
  {
    ...
    title: "The 'Bionic Partition': Generative Design of a 45% Lighter Cabin Component",
    problem:
      "Cabin partitions - the structural walls separating passenger seating from galleys - are heavy fixed components. On the A320, the standard partition weighed roughly 65 kg. With thousands of A320-family aircraft in the backlog, every kilogram of structural weight compounds into lifetime fuel burn and CO2 across the fleet, so Airbus wanted a radically lighter partition that still met crashworthiness and certification loads.",
    solution:
      "Working with Autodesk and its research studio The Living, Airbus applied generative design (algorithmic topology optimization) to the partition. Engineers specified the load cases, mounting points, and material constraints, and the software generated thousands of organically-branched lattice structures - inspired by slime-mould growth patterns and bone structure - then down-selected the strongest-per-gram candidates. The winning geometry was manufactured in a scalmalloy metal alloy using additive manufacturing (3D printing).",
    outcome:
      "The generatively-designed 'bionic partition' was around 45% lighter than the traditional part (a reduction of roughly 30 kg per unit) while meeting the same structural requirements. Airbus reported that fleet-wide adoption of such weight-optimized components could remove meaningful mass per aircraft, cutting fuel consumption and CO2 over the aircraft's service life. The project became a flagship demonstration of generative design and additive manufacturing in aerospace.",
    metrics: [
      "~45% lighter than the conventional partition",
      "~30 kg saved per partition unit",
      "Manufactured via metal 3D printing in scalmalloy",
      "Thousands of design candidates generated algorithmically",
    ],
    tags: ["Manufacturing", "Generative Design", "Engineering", "Aerospace", "Additive Manufacturing"],
    featured: true,
    businessContext:
      "Airbus faces a large A320-family order backlog and sustained pressure to reduce aircraft weight for fuel efficiency and emissions. Structural components like cabin partitions had been optimized within traditional design and machining limits for decades. Generative design combined with additive manufacturing opened a new design space - geometries too complex to machine but printable - allowing Airbus to explore weight reductions that conventional CAD workflows could not reach.",
    strategicDrivers: [
      "Large A320-family backlog where per-aircraft weight savings compound across the fleet",
      "Fuel efficiency and CO2 reduction targets driving aggressive lightweighting",
      "Emergence of industrial metal additive manufacturing enabling previously unmanufacturable geometries",
      "Desire to demonstrate and mature generative design as a repeatable aerospace engineering method",
    ],
    techStack: [
      "Autodesk generative design / topology optimization tools",
      "Autodesk research collaboration (The Living studio)",
      "Scalmalloy high-strength aluminium-magnesium-scandium alloy",
      "Metal additive manufacturing (selective laser melting)",
      "Finite element analysis for load-case validation",
      "CAD/CAM toolchain for print preparation",
    ],
    architecture:
      "Engineers define the design problem as a set of boundary conditions: fixed mounting points, structural load cases (including crash loads), material properties, and a mass objective. The generative design algorithm iteratively grows and prunes structural members - producing many organically-branched lattice candidates that route material only along load paths. Candidates are ranked by strength-to-weight and manufacturability, validated with finite element analysis, and the selected geometry is prepared for additive manufacturing rather than conventional machining.",
    dataRequirements:
      "Structural load-case definitions and certification load requirements for the partition, material property data for scalmalloy, and geometric constraints (envelope, attachment interfaces). The generative approach relies on physics-based simulation rather than large historical training datasets, so the primary inputs are engineering constraints and material models rather than proprietary customer data.",
    investmentEstimate: "Not publicly disclosed - an R&D collaboration between Airbus and Autodesk",
    annualReturn: "Not publicly disclosed - value is primarily fleet fuel/CO2 savings from lightweighting plus method maturation",
    paybackPeriod: "Not publicly disclosed",
    roiMultiple: "Not publicly disclosed",
    roiBreakdown: [
      {
        category: "Weight reduction",
        value: "~45% lighter part",
        note: "~30 kg saved per partition; lighter structure reduces lifetime fuel burn and CO2 per aircraft",
      },
      {
        category: "Method & capability value",
        value: "Not disclosed",
        note: "Established generative design + additive manufacturing as a reusable aerospace engineering approach for future components",
      },
    ],
    implementationTimeline: "Developed and publicly demonstrated around 2015-2016 as an Airbus-Autodesk R&D project",
    implementationPhases: [
      {
        phase: "Problem Definition & Constraints",
        duration: "Initial phase",
        description:
          "Airbus engineers translated the partition's structural requirements - mounting points, crash and operational load cases, and material limits - into a formal generative design problem.",
        keyOutputs: ["Load-case specification", "Geometric envelope and interfaces", "Mass-reduction objective"],
      },
      {
        phase: "Generative Design Exploration",
        duration: "Iterative",
        description:
          "Using Autodesk generative tools, the team generated thousands of lattice candidates inspired by natural growth structures, then down-selected on strength-to-weight and manufacturability.",
        keyOutputs: ["Thousands of candidate geometries", "Down-selected lattice design", "FEA validation"],
      },
      {
        phase: "Additive Manufacturing & Validation",
        duration: "Prototype",
        description:
          "The selected geometry was 3D-printed in scalmalloy and validated against structural requirements, demonstrating the ~45% weight reduction versus the conventional partition.",
        keyOutputs: ["Printed bionic partition", "Structural validation", "Public demonstrator"],
      },
    ],
    teamSize: "Joint Airbus and Autodesk (The Living) engineering and research team",
    challenges: [
      "Certification of additively-manufactured structural aerospace parts required extensive material and process qualification",
      "Metal additive manufacturing at production scale and repeatability was still maturing",
      "Generatively-designed organic geometries are difficult to inspect and validate with traditional methods",
      "Integrating generative design into established aerospace engineering workflows required new skills and tooling",
    ],
    governanceFramework: [
      "Structural designs validated against certification load cases before acceptance",
      "Material and additive-manufacturing process qualification for aerospace use",
      "Engineering sign-off on all generatively-produced geometries",
    ],
    dataPrivacy: [
      "Project centered on structural engineering data rather than personal data",
      "Design and material data handled within Airbus and partner engineering environments",
    ],
    humanOversight:
      "Generative design produced candidate geometries, but Airbus engineers defined the constraints, selected the final design, and remained responsible for structural validation and certification. The algorithm expanded the design space; engineering judgment governed what was accepted.",
    regulatoryConsiderations: [
      "EASA/FAA structural certification requirements for cabin components",
      "Material and process qualification for additively-manufactured aerospace parts",
    ],
    lessonsLearned: [
      "Generative design plus additive manufacturing can unlock weight savings unreachable with conventional design and machining",
      "Nature-inspired lattice structures route material efficiently along load paths",
      "Qualifying new materials and additive processes is often the pacing item, not the design algorithm itself",
      "A high-visibility demonstrator component is an effective way to mature and prove a new engineering method",
    ],
    whatWorkedWell: [
      "Partnering with Autodesk brought mature generative design tooling and research expertise",
      "Framing the partition as a bounded, high-impact component made it an ideal proving ground",
      "Combining generative geometry with additive manufacturing let the team realize designs that could not otherwise be built",
    ],
    references: [
      {
        label: "Autodesk: Reimagining the Future of Air Travel (bionic partition)",
        url: "https://www.autodesk.com/customer-stories/airbus",
      },
      {
        label: "Airbus: Pioneering bionic 3D printing",
        url: "https://www.airbus.com/en/newsroom/news/2016-03-pioneering-bionic-3d-printing",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    references: [
      ...
      {
        label: "GitHub Research: Copilot productivity study",
